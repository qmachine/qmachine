...
TEMPLATE_ROOT="${TEMPLATE_ROOT:=${PWD}}";

generate_config ${COUCHDB_INI:=/dev/null} <<-EOF
    [couchdb]
    database_dir = ${TEMPLATE_ROOT}/var/couchdb
    view_index_dir = ${TEMPLATE_ROOT}/var/couchdb
    uri_file = ${TEMPLATE_ROOT}/var/couchdb/couch.uri
    ...
    [log]
    file = ${TEMPLATE_ROOT}/var/couchdb/couch.log
    include_sasl = true
    level = info
EOF
    ...
    <plist version="1.0">
      <dict>
        ...
        <array>
          ...
          <string>-a</string>
          <string>${TEMPLATE_ROOT}/var/couchdb.ini</string>
          <string>-p</string>
          <string>${TEMPLATE_ROOT}/var/couchdb/couch.pid</string>
        </array>
        <key>RunAtLoad</key>
        <true/>
        <key>StandardOutPath</key>
        <string>${TEMPLATE_ROOT}/var/couchdb/stdout.txt</string>
        <key>StandardErrorPath</key>
        <string>${TEMPLATE_ROOT}/var/couchdb/stderr.txt</string>
        <key>UserName</key>
...
generate_config ${NODEJS_PLIST:=/dev/null} <<-EOF
    ...
    <plist version="1.0">
      <dict>
        ...
        <true/>
        <key>StandardOutPath</key>
        <string>${TEMPLATE_ROOT}/var/nodejs/stdout.txt</string>
        <key>StandardErrorPath</key>
        <string>${TEMPLATE_ROOT}/var/nodejs/stderr.txt</string>
        <key>UserName</key>
        <string>${USERNAME}</string>
        <key>WorkingDirectory</key>
        <string>${TEMPLATE_ROOT}/var/nodejs/</string>
      </dict>
    </plist>
EOF
